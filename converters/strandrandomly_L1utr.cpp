/*
 This code adds reading from to the L1utr grammar containing only forward letters.
	...
	for (int j=0; j<unstrandedStk[0].sequence.size(); j++) {
		...
		if (strand == 1) {
			...
			for (int i=0; i<numstklines; i++) {
				...
				if (unstrandedStk[i].sequence[j] == '5') {
					strandedStk[i].sequence += '$';
					continue;
				}
				if (unstrandedStk[i].sequence[j] == '6') {
					strandedStk[i].sequence += '%';
					continue;
				}
				if (unstrandedStk[i].sequence[j] == '~') {
					strandedStk[i].sequence += '0';
					continue;
				}
				if (unstrandedStk[i].sequence[j] == 'h') {
					strandedStk[i].sequence += 'M';
					continue;
				}
				if (unstrandedStk[i].sequence[j] == 'l') {
				...
				if (unstrandedStk[i].sequence[j] == '3') {
					strandedStk[i].sequence += '&';
					continue;
				}
				if (unstrandedStk[i].sequence[j] == '4') {
					strandedStk[i].sequence += '\'';
					continue;
				}
				if (unstrandedStk[i].sequence[j] == 'z') {
					strandedStk[i].sequence += '\\';
					continue;
				}
				if (unstrandedStk[i].sequence[j] == 'm') {
